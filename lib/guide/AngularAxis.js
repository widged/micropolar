define(['d3', 'accessmaker'], function(d3, AccessMaker) {
    ...
    var Class = function AngularGuide() {
        ...
        var state = {
            ...
            radius: 250,
            originTheta: -90,
            ticks: {majorQty: 0, minorQty: 0, labelOffset: 6, suffix: '', range: [], orientation: 'angular' },
        };
        ...
        instance.mountIn = function(rootNode) {
            ...
            var ticksConfig  = state.ticks;

            var axisNode = rootNode.select('.angular.axis');

            var currentAngle = function(d, i){
                return (axisCoord(d.value) + originTheta) % 360;
            };

            renderMarks(axisNode, ticksConfig, currentAngle, radius);
            renderLabels(axisNode, ticksConfig, currentAngle, radius);
